...
This is presentation control. A user who disables JavaScript can still see the card, and direct access remains allowed by design in 1.1.0. GLPI remains responsible for all form permissions.
...
Configuration is global in 1.1.0. Assets are served from the plugin's GLPI 11 `public/` directory and loaded on every `/ServiceCatalog` request. The warning is triggered by the configured `category` query parameter or by clicking its category card. The on-call form can only be selected when it is active, non-deleted, visible in the administrator's active entity scope, and answerable under native access policies. The holiday calendar is persisted as validated JSON in the same configuration context and overrides business hours for matching local dates.
